...
- 공동작업자가 "올려줘/공유해줘"라고 하면 `main`에 직접 push하지 않는다. 새 브랜치를 만들어 "파일 배치 규칙"대로 정리·커밋한 뒤 **PR을 연다**(흐름: `docs/COLLABORATION.md`의 "자료 올리기" 절 / `go-github` 스킬 = `/go-github`). 공동작업자 PR 병합은 김성균 검토 후.
